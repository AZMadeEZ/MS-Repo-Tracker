# Changes on default branch since 2026-06-07T21:38:00.975882Z

Repos with movement: **68**

## docs

### Azure

- **Azure/azure-sdk-for-net** (`main`) - **13** commit(s)
  - [Regenerate repository SDK with TypeSpec build 20260608.3 (#59750)](https://github.com/Azure/azure-sdk-for-net/commit/1c920c91680d27e712ffa2e3cb6b7a33a79fbe73) - 2026-06-08T20:01:39Z - azure-sdk-automation[bot] (@azure-sdk-automation[bot]) - PR: [Update azure-typespec/http-client-csharp version to prerelease 1.0.0-alpha.20260608.3 #59750](https://github.com/Azure/azure-sdk-for-net/pull/59750)
  - [Update UnbrandedGeneratorVersion to 1.0.0-alpha.20260608.2 (#59747)](https://github.com/Azure/azure-sdk-for-net/commit/709a6aeaad37d9d32808f1a429d070983ad971af) - 2026-06-08T18:14:20Z - azure-sdk-automation[bot] (@azure-sdk-automation[bot]) - PR: [Update UnbrandedGeneratorVersion to 1.0.0-alpha.20260608.2 #59747](https://github.com/Azure/azure-sdk-for-net/pull/59747)
  - [Update azure-typespec/http-client-csharp version to prerelease 1.0.0-…](https://github.com/Azure/azure-sdk-for-net/commit/b3baa956632b6d24b2fb940c4ff0e9bb66681e47) - 2026-06-08T17:29:23Z - azure-sdk-automation[bot] (@azure-sdk-automation[bot]) - PR: [Update azure-typespec/http-client-csharp version to prerelease 1.0.0-alpha.20260608.2 #59744](https://github.com/Azure/azure-sdk-for-net/pull/59744)
  - [Generate a new code from refactored typespec (#59722)](https://github.com/Azure/azure-sdk-for-net/commit/fec47cef4fa13165cdeee669668450f9608ab41e) - 2026-06-08T15:52:54Z - Nikolay Rovinskiy (@nick863) - PR: [Generate a new code from refactored typespec #59722](https://github.com/Azure/azure-sdk-for-net/pull/59722)
  - [Add sample, readme and test for External Agent (#59727)](https://github.com/Azure/azure-sdk-for-net/commit/2af2c29accbf4a306da98937b2fd513f90d42784) - 2026-06-08T15:52:42Z - Nikolay Rovinskiy (@nick863) - PR: [Add sample, readme and test for External Agent #59727](https://github.com/Azure/azure-sdk-for-net/pull/59727)

- **Azure/azure-sdk-for-java** (`main`) - **9** commit(s)
  - [Updating netty (#49416)](https://github.com/Azure/azure-sdk-for-java/commit/b95c680f97848d46a4b9bd376b24bce86f258fce) - 2026-06-08T20:55:02Z - Jair Myree (@jairmyree) - PR: [Updating Netty Dependencies #49416](https://github.com/Azure/azure-sdk-for-java/pull/49416)
  - [Configurations:  'specification/postgresql/DBforPostgreSQL.Management…](https://github.com/Azure/azure-sdk-for-java/commit/b1ec6ede98325f4b6eb5c9dbe86b80caf049674f) - 2026-06-08T20:50:19Z - Azure SDK Bot (@azure-sdk) - PR: [[AutoPR azure-resourcemanager-postgresqlflexibleserver]-generated-from-SDK Generation - Java-6390688 #49366](https://github.com/Azure/azure-sdk-for-java/pull/49366)
  - [Cosmos Release 2026-06-08 (#49407)](https://github.com/Azure/azure-sdk-for-java/commit/a18aefd1f3a7392a166274092db4cc15986a2eca) - 2026-06-08T19:21:17Z - Fabian Meiswinkel (@FabianMeiswinkel) - PR: [Cosmos Release 2026-06-08 #49407](https://github.com/Azure/azure-sdk-for-java/pull/49407)
  - [Updating Missing Telemetry (#48978)](https://github.com/Azure/azure-sdk-for-java/commit/f8877c0abd43da6e2e769c8e77e7d6a51a7e6774) - 2026-06-08T18:25:13Z - Matthew Metcalf (@mrm9084) - PR: [Updating Missing Telemetry #48978](https://github.com/Azure/azure-sdk-for-java/pull/48978)
  - [[confidentialledger] Enforce stricter redirect destination policy (#4…](https://github.com/Azure/azure-sdk-for-java/commit/387345eef6d7fbbc6bef0c8fe2affff566dc9aea) - 2026-06-08T17:08:19Z - Ryan Zhang (@ryazhang-microsoft) - PR: [[confidentialledger] Enforce stricter redirect destination policy #49391](https://github.com/Azure/azure-sdk-for-java/pull/49391)

- **Azure/azure-sdk-for-js** (`main`) - **8** commit(s)
  - [[VoiceLive] Add live telemetry test for realtime session (#38868)](https://github.com/Azure/azure-sdk-for-js/commit/5b682289a6c409fbf94905e87dbe0159dcaf604a) - 2026-06-08T21:28:47Z - xitzhang (@xitzhang) - PR: [[VoiceLive] Add live telemetry test for realtime session #38868](https://github.com/Azure/azure-sdk-for-js/pull/38868)
  - [[VoiceLive] Add live test coverage and parametrize over API versions …](https://github.com/Azure/azure-sdk-for-js/commit/dc5f9743b05ea78f938eba31cbe704150889490c) - 2026-06-08T18:13:24Z - xitzhang (@xitzhang) - PR: [[VoiceLive] Add live test coverage and parametrize over API versions #38790](https://github.com/Azure/azure-sdk-for-js/pull/38790)
  - [Update `@azure/openai` README links for Build Analyze link-check fail…](https://github.com/Azure/azure-sdk-for-js/commit/bc6fb64cb063a84a87d4d0a29e9b63433e9c58d3) - 2026-06-08T18:11:03Z - Copilot (@Copilot) - PR: [Update `@azure/openai` README links for Build Analyze link-check failures #38809](https://github.com/Azure/azure-sdk-for-js/pull/38809)
  - [[EngSys] automatic pnpm update (#38861)](https://github.com/Azure/azure-sdk-for-js/commit/09add7996feef1215e7a5f99d883fed8c39a3c6d) - 2026-06-08T16:02:58Z - azure-sdk-automation[bot] (@azure-sdk-automation[bot]) - PR: [[EngSys] automatic pnpm update #38861](https://github.com/Azure/azure-sdk-for-js/pull/38861)
  - [[AutoPR @azure-arm-postgresql-flexible]-generated-from-SDK Generation…](https://github.com/Azure/azure-sdk-for-js/commit/9637114573de5b7bc402209b444cb8e2bf035884) - 2026-06-08T15:31:04Z - Azure SDK Bot (@azure-sdk) - PR: [[AutoPR @azure-arm-postgresql-flexible]-generated-from-SDK Generation - JS-6405188 #38808](https://github.com/Azure/azure-sdk-for-js/pull/38808)

- **Azure/azure-sdk-for-python** (`main`) - **5** commit(s)
  - [Bump changelogs after release (#47398)](https://github.com/Azure/azure-sdk-for-python/commit/c093c23f3ae6976810c53cc41b073705ecf4aceb) - 2026-06-08T21:23:38Z - Peter Wu (@weirongw23-msft) - PR: [[Storage] Bump Changelogs after STG 102 Release #47398](https://github.com/Azure/azure-sdk-for-python/pull/47398)
  - [[AutoPR azure-mgmt-postgresqlflexibleservers]-generated-from-SDK Gene…](https://github.com/Azure/azure-sdk-for-python/commit/e942181cdfa23157033fa18d76d659ba7c8ce55e) - 2026-06-08T20:50:14Z - Azure SDK Bot (@azure-sdk) - PR: [[AutoPR azure-mgmt-postgresqlflexibleservers]-generated-from-SDK Generation - Python-6390684 #47321](https://github.com/Azure/azure-sdk-for-python/pull/47321)
  - [[confidentialledger] Enforce stricter redirect destination policy (#4…](https://github.com/Azure/azure-sdk-for-python/commit/2fb56bf4f962171f7eb7efc6daad23bf610ae906) - 2026-06-08T19:23:44Z - Ryan Zhang (@ryazhang-microsoft) - PR: [[confidentialledger] Enforce stricter redirect destination policy #47368](https://github.com/Azure/azure-sdk-for-python/pull/47368)
  - [Increment package version after release of azure-monitor-opentelemetr…](https://github.com/Azure/azure-sdk-for-python/commit/321370a7c625eb3c61d0ae849cea1ad5e836c1a8) - 2026-06-08T16:42:10Z - Azure SDK Bot (@azure-sdk) - PR: [Increment version for monitor releases #47394](https://github.com/Azure/azure-sdk-for-python/pull/47394)
  - [Exporter release 1.0.0b53 (#47296)](https://github.com/Azure/azure-sdk-for-python/commit/6f3980e81598b271c5a3ac3912e9da67e2d1b479) - 2026-06-08T14:24:26Z - Radhika Gupta (@rads-1996) - PR: [Exporter release 1.0.0b53 #47296](https://github.com/Azure/azure-sdk-for-python/pull/47296)

- **Azure/azure-sdk-for-rust** (`main`) - **2** commit(s)
  - [Preserve cached routing map on transient pkrange fetch failure (#4549)](https://github.com/Azure/azure-sdk-for-rust/commit/e08f9f5933a9a3a9ddf9ece3c3e1cd6b3c68b40e) - 2026-06-08T21:08:45Z - Tomas Varon (@tvaron3) - PR: [Preserve cached routing map on transient pkrange fetch failure #4549](https://github.com/Azure/azure-sdk-for-rust/pull/4549)
  - [Hand off @ronniegeraghty CODEOWNERS to new owners (#4499)](https://github.com/Azure/azure-sdk-for-rust/commit/b080e84933f54501751f5416de23a7ce7e717201) - 2026-06-08T20:14:28Z - Ronnie Geraghty (@ronniegeraghty) - PR: [Hand off @ronniegeraghty CODEOWNERS to new owners #4499](https://github.com/Azure/azure-sdk-for-rust/pull/4499)

- **Azure/azure-sdk-for-go** (`main`) - **2** commit(s)
  - [Configurations:  'specification/postgresql/DBforPostgreSQL.Management…](https://github.com/Azure/azure-sdk-for-go/commit/410af737d35baadce77aa822f2f16b54ca7f271c) - 2026-06-08T20:50:25Z - Azure SDK Bot (@azure-sdk) - PR: [[AutoPR sdk-resourcemanager/postgresql/armpostgresqlflexibleservers]-generated-from-SDK Generation - Go-6390685 #26936](https://github.com/Azure/azure-sdk-for-go/pull/26936)
  - [[Automation] Regenerate SDK based on typespec-go branch main 【batch 8…](https://github.com/Azure/azure-sdk-for-go/commit/71c05849d5e04e52315982dce30abe2f19226316) - 2026-06-08T07:25:49Z - Azure SDK Bot (@azure-sdk) - PR: [[Automation] Regenerate SDK based on typespec-go branch main 【batch 8】 #26955](https://github.com/Azure/azure-sdk-for-go/pull/26955)

- **Azure/iot-operations-sdks** (`main`) - **1** commit(s)
  - [[ADR] mRPC Back Pressure flag (#1332)](https://github.com/Azure/iot-operations-sdks/commit/597532eac5a081a76b04772efb9922ff5301fa43) - 2026-06-08T21:27:12Z - Maxim Semenov (@maximsemenov80) - PR: [[ADR] mRPC Back Pressure flag #1332](https://github.com/Azure/iot-operations-sdks/pull/1332)

- **Azure/data-api-builder** (`main`) - **1** commit(s)
  - [Mark required stored-procedure parameters as NON_NULL in GraphQL (#3646)](https://github.com/Azure/data-api-builder/commit/5b538c610c5cf80d239f5d53d0239a5996e20895) - 2026-06-08T01:42:49Z - Anusha Kolan (@anushakolan) - PR: [Mark required stored-procedure parameters as NON_NULL in GraphQL #3646](https://github.com/Azure/data-api-builder/pull/3646)

### MicrosoftDocs

- **MicrosoftDocs/azure-docs-powershell** (`main`) - **40** commit(s)
  - [status (#4198)](https://github.com/MicrosoftDocs/azure-docs-powershell/commit/f02f47d25368ce31d85633a464ccd34e15bb4ffe) - 2026-06-08T17:40:08Z - learn-build-service-prod[bot] (@learn-build-service-prod[bot]) - PR: [[BULK] [Bundle-Security] - Scheduled execution to fix known issues (part 71) #4198](https://github.com/MicrosoftDocs/azure-docs-powershell/pull/4198)
  - [status (#4197)](https://github.com/MicrosoftDocs/azure-docs-powershell/commit/84ff327183f88edf52637219fb3bbecab903dc18) - 2026-06-08T17:40:04Z - learn-build-service-prod[bot] (@learn-build-service-prod[bot]) - PR: [[BULK] [Bundle-Security] - Scheduled execution to fix known issues (part 70) #4197](https://github.com/MicrosoftDocs/azure-docs-powershell/pull/4197)
  - [status (#4196)](https://github.com/MicrosoftDocs/azure-docs-powershell/commit/05e4ea9869085f506861b735630a2bc91b7b2bc9) - 2026-06-08T17:40:00Z - learn-build-service-prod[bot] (@learn-build-service-prod[bot]) - PR: [[BULK] [Bundle-Security] - Scheduled execution to fix known issues (part 69) #4196](https://github.com/MicrosoftDocs/azure-docs-powershell/pull/4196)
  - [status (#4195)](https://github.com/MicrosoftDocs/azure-docs-powershell/commit/4bc6f3e196c956b5dde47efcba004aa5c546c6e2) - 2026-06-08T17:39:56Z - learn-build-service-prod[bot] (@learn-build-service-prod[bot]) - PR: [[BULK] [Bundle-Security] - Scheduled execution to fix known issues (part 68) #4195](https://github.com/MicrosoftDocs/azure-docs-powershell/pull/4195)
  - [status (#4194)](https://github.com/MicrosoftDocs/azure-docs-powershell/commit/e27182e88dd872f07fc88b41605f360f63e76ce2) - 2026-06-08T17:39:52Z - learn-build-service-prod[bot] (@learn-build-service-prod[bot]) - PR: [[BULK] [Bundle-Security] - Scheduled execution to fix known issues (part 67) #4194](https://github.com/MicrosoftDocs/azure-docs-powershell/pull/4194)

- **MicrosoftDocs/PowerShell-Docs-Modules** (`main`) - **3** commit(s)
  - [[Freshness] PSScriptAnalyzer Article Updates - Batch 9 (#411)](https://github.com/MicrosoftDocs/PowerShell-Docs-Modules/commit/79305774e2bc60fbb8a9bb831bf4fdaefaeafeea) - 2026-06-08T18:16:00Z - Xelu86 (@Xelu86) - PR: [[Freshness] PSScriptAnalyzer Article Updates - Batch 9 #411](https://github.com/MicrosoftDocs/PowerShell-Docs-Modules/pull/411)
  - [[Freshness] PSScriptAnalyzer Article Updates - Batch 8 (#409)](https://github.com/MicrosoftDocs/PowerShell-Docs-Modules/commit/b471075ef349b3efd2f724cb816eb4805258dd7f) - 2026-06-08T18:11:39Z - Xelu86 (@Xelu86) - PR: [[Freshness] PSScriptAnalyzer Article Updates - Batch 8 #409](https://github.com/MicrosoftDocs/PowerShell-Docs-Modules/pull/409)
  - [[Freshness] PSScriptAnalyzer Article Updates - Batch 7 (#408)](https://github.com/MicrosoftDocs/PowerShell-Docs-Modules/commit/67ac3e9d59a5d10be15d3a412b3c3795d5a401d4) - 2026-06-08T18:09:56Z - Xelu86 (@Xelu86) - PR: [[Freshness] PSScriptAnalyzer Article Updates - Batch 7 #408](https://github.com/MicrosoftDocs/PowerShell-Docs-Modules/pull/408)

- **MicrosoftDocs/azure-docs-cli** (`main`) - **2** commit(s)
  - [docs: fix bicep publish force example (#6015)](https://github.com/MicrosoftDocs/azure-docs-cli/commit/36ffb1d285ffacf59ccc9244415f903dce6eab4b) - 2026-06-08T17:13:55Z - titanniya542-spec (@titanniya542-spec) - PR: [docs: fix bicep publish force example #6015](https://github.com/MicrosoftDocs/azure-docs-cli/pull/6015)
  - [Update CLI extensions available doc. (#6014)](https://github.com/MicrosoftDocs/azure-docs-cli/commit/319807ffb1f715ddc51ed48d1c6b218c1eedb405) - 2026-06-08T17:06:48Z - Azure CLI Bot (@azclibot) - PR: [Update CLI extensions available doc #6014](https://github.com/MicrosoftDocs/azure-docs-cli/pull/6014)

- **MicrosoftDocs/webview2-winrt-reference** (`main`) - **2** commit(s)
  - [WebView2: Update documentation for 150.0.4071.0 (#121)](https://github.com/MicrosoftDocs/webview2-winrt-reference/commit/64296c177d21b165fd0c2d6f25185a6aacf30836) - 2026-06-08T14:21:34Z - Ojasvi Das (@ojasvi1450) - PR: [WebView2: Update documentation for 150.0.4071.0 #121](https://github.com/MicrosoftDocs/webview2-winrt-reference/pull/121)
  - [WebView2: Update documentation for 149.0.4022.49 (#120)](https://github.com/MicrosoftDocs/webview2-winrt-reference/commit/21bd9a632f178762c44de35711f8e167720248c1) - 2026-06-08T09:39:54Z - Ojasvi Das (@ojasvi1450) - PR: [WebView2: Update documentation for 149.0.4022.49 #120](https://github.com/MicrosoftDocs/webview2-winrt-reference/pull/120)

- **MicrosoftDocs/webview2-win32-reference** (`main`) - **2** commit(s)
  - [WebView2: Update documentation for 150.0.4071.0 (#144)](https://github.com/MicrosoftDocs/webview2-win32-reference/commit/a1aa09b6dfbe49cdc84ac125d7f4c78b1c1a5d9d) - 2026-06-08T14:18:43Z - Ojasvi Das (@ojasvi1450) - PR: [WebView2: Update documentation for 150.0.4071.0 #144](https://github.com/MicrosoftDocs/webview2-win32-reference/pull/144)
  - [WebView2: Update documentation for 149.0.4022.49 (#143)](https://github.com/MicrosoftDocs/webview2-win32-reference/commit/50a0c03a3c3014793f9f0f5cd975eafb319c9c4c) - 2026-06-08T09:38:55Z - Ojasvi Das (@ojasvi1450) - PR: [WebView2: Update documentation for 149.0.4022.49 #143](https://github.com/MicrosoftDocs/webview2-win32-reference/pull/143)

- **MicrosoftDocs/sql-docs** (`live`) - **1** commit(s)
  - [Update target-platform.md - add Sql170DatabaseSchemaProvider (#10346)](https://github.com/MicrosoftDocs/sql-docs/commit/b3f5d4dec3aa52912e8bad7abfdc6e00893bcf68) - 2026-06-08T20:16:41Z - Ohad Tsamir @ Independer (@otsamir) - PR: [Update target-platform.md - add Sql170DatabaseSchemaProvider #10346](https://github.com/MicrosoftDocs/sql-docs/pull/10346)

- **MicrosoftDocs/azure-aks-docs** (`main`) - **1** commit(s)
  - [cluster extensions - updates for core extensions (#432)](https://github.com/MicrosoftDocs/azure-aks-docs/commit/70aae02f8eee515aae2af2a6b1e8b875b51c5c31) - 2026-06-08T14:09:59Z - Pradeep Agarwal (@pradeepjsr05) - PR: [cluster extensions - updates for core extensions #432](https://github.com/MicrosoftDocs/azure-aks-docs/pull/432)

### dotnet

- **dotnet/runtime** (`main`) - **25** commit(s)
  - [Implement Environment.WorkingSet on OpenBSD (#129078)](https://github.com/dotnet/runtime/commit/5416bfe7ce1291da637aa119738a300f013569db) - 2026-06-08T21:34:04Z - Adeel Mujahid (@am11) - PR: [Implement Environment.WorkingSet on OpenBSD #129078](https://github.com/dotnet/runtime/pull/129078)
  - [Add OpenBSD support for ICU library loading (#129124)](https://github.com/dotnet/runtime/commit/fdec0235c30ece9bd88b221522ac2aa0131d8418) - 2026-06-08T21:20:58Z - Adeel Mujahid (@am11) - PR: [Add OpenBSD support for ICU library loading #129124](https://github.com/dotnet/runtime/pull/129124)
  - [Implement getexepath for OpenBSD (#129103)](https://github.com/dotnet/runtime/commit/e3d5aac6afa2f26837e4cfcb0647fed3e8e0f82b) - 2026-06-08T21:11:57Z - Adeel Mujahid (@am11) - PR: [Implement getexepath for OpenBSD #129103](https://github.com/dotnet/runtime/pull/129103)
  - [[Wasm R2R] Allow LDVIRTFTN call sigs to carry a type arg (MD array me…](https://github.com/dotnet/runtime/commit/826a0c6a8502849b1290a4865535eef35962b3a1) - 2026-06-08T21:05:15Z - Andy Ayers (@AndyAyersMS) - PR: [[Wasm R2R] Allow LDVIRTFTN call sigs to carry a type arg (MD array methods) #129064](https://github.com/dotnet/runtime/pull/129064)
  - [Arm64:[PAC-RET] Use hint-space instruction for stripping return addre…](https://github.com/dotnet/runtime/commit/dcabbab1b8ffeb455ee31fd47eea822040be0960) - 2026-06-08T20:38:22Z - SwapnilGaikwad (@SwapnilGaikwad) - PR: [Arm64:[PAC-RET] Use hint-space instruction for stripping return address #129130](https://github.com/dotnet/runtime/pull/129130)

- **dotnet/msbuild** (`main`) - **14** commit(s)
  - [Bump actions/github-script from 7 to 9 (#14004)](https://github.com/dotnet/msbuild/commit/362fdf7843d03e8f40053b3378495a88ffb1916d) - 2026-06-08T18:27:09Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump actions/github-script from 7 to 9 #14004](https://github.com/dotnet/msbuild/pull/14004)
  - [Bump actions/cache from 4 to 5 (#14001)](https://github.com/dotnet/msbuild/commit/9df212006bcd5313c19cea541f5a00b58b3237cf) - 2026-06-08T17:36:14Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump actions/cache from 4 to 5 #14001](https://github.com/dotnet/msbuild/pull/14001)
  - [Bump actions/download-artifact from 4 to 8 (#14000)](https://github.com/dotnet/msbuild/commit/ead9a1f0e2c43c9902d7a693fbef3d142454d500) - 2026-06-08T17:23:57Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump actions/download-artifact from 4 to 8 #14000](https://github.com/dotnet/msbuild/pull/14000)
  - [Enabling weekly Dependabot updates for GitHub Actions (#13999)](https://github.com/dotnet/msbuild/commit/3398824f5962a82d96381aa6cc2c00d2a9ef9a4e) - 2026-06-08T15:43:29Z - Copilot (@Copilot) - PR: [Enabling weekly Dependabot updates for GitHub Actions #13999](https://github.com/dotnet/msbuild/pull/13999)
  - [chore: Pin GitHub Actions to commit SHAs (#13269)](https://github.com/dotnet/msbuild/commit/4dbca8e0a67bc7b84329f9b6bc54d5264714af56) - 2026-06-08T14:20:15Z - Pádraic Slattery (@pgoslatara) - PR: [chore: Pin GitHub Actions to commit SHAs #13269](https://github.com/dotnet/msbuild/pull/13269)

- **dotnet/orleans** (`main`) - **6** commit(s)
  - [fix: make bank transfer fault test deterministic (#10185)](https://github.com/dotnet/orleans/commit/6a98474dda3b7548af4f93c041f4dad1e7097147) - 2026-06-08T21:01:24Z - Reuben Bond (@ReubenBond) - PR: [fix: make bank transfer fault test deterministic #10185](https://github.com/dotnet/orleans/pull/10185)
  - [chore: use IMeterFactory for MessagingProcessingInstruments (#10180)](https://github.com/dotnet/orleans/commit/a1c304923c4a05c7b9374a23d993186f2226ab84) - 2026-06-08T20:07:53Z - Meir Blachman (@Meir017) - PR: [chore: use IMeterFactory for MessagingProcessingInstruments #10180](https://github.com/dotnet/orleans/pull/10180)
  - [chore: use IMeterFactory for AzureBlobJournalStorageInstruments (#10183)](https://github.com/dotnet/orleans/commit/b413d0ef63b5a568fbcb136923be086def6c61f2) - 2026-06-08T20:05:09Z - Meir Blachman (@Meir017) - PR: [chore: use IMeterFactory for AzureBlobJournalStorageInstruments #10183](https://github.com/dotnet/orleans/pull/10183)
  - [fix(redis): consistent grain directory disposal + multiplexer ownersh…](https://github.com/dotnet/orleans/commit/99d24556ce11459168e6d557e828eba0e0ca081a) - 2026-06-08T19:16:28Z - Reuben Bond (@ReubenBond) - PR: [fix(redis): consistent grain directory disposal + multiplexer ownership tests #10182](https://github.com/dotnet/orleans/pull/10182)
  - [chore: use IMeterFactory for JournalingInstruments (#10184)](https://github.com/dotnet/orleans/commit/78809153c0ad4665520475356dc90e3b4398ba92) - 2026-06-08T19:03:23Z - Meir Blachman (@Meir017) - PR: [chore: use IMeterFactory for JournalingInstruments #10184](https://github.com/dotnet/orleans/pull/10184)

- **dotnet/roslyn** (`main`) - **6** commit(s)
  - [Increase timeout for Correctness_Analyzers job (#84064)](https://github.com/dotnet/roslyn/commit/576f18b3e54f0511c2fc93a2b20d2762148b688b) - 2026-06-08T20:04:06Z - Jared Parsons (@jaredpar) - PR: [Increase timeout for Correctness_Analyzers job #84064](https://github.com/dotnet/roslyn/pull/84064)
  - [Fix mishandling of large embedded C# literals (#84043)](https://github.com/dotnet/roslyn/commit/644b5da46ba33d4057d82b7629e8def3b88dae64) - 2026-06-08T17:49:22Z - Jason Malinowski (@jasonmalinowski) - PR: [Fix mishandling of large embedded C# literals #84043](https://github.com/dotnet/roslyn/pull/84043)
  - [[main] Update dependencies from dotnet/roslyn (#84058)](https://github.com/dotnet/roslyn/commit/82ad1e15e364edbd6ca59d8a8325df2b6488e28e) - 2026-06-08T16:12:49Z - dotnet-maestro[bot] (@dotnet-maestro[bot]) - PR: [[main] Update dependencies from dotnet/roslyn #84058](https://github.com/dotnet/roslyn/pull/84058)
  - [Unsafe evolution: propagate unsafe context to constructor initializer…](https://github.com/dotnet/roslyn/commit/d48095f632dad4d30b44d2b4d6a84163e3101024) - 2026-06-08T13:02:14Z - Jan Jones (@jjonescz) - PR: [Unsafe evolution: propagate unsafe context to constructor initializer #83969](https://github.com/dotnet/roslyn/pull/83969)
  - [Parameterize DartLab templates repository alias (#84037)](https://github.com/dotnet/roslyn/commit/8f825c9697554bd194c542154a6026b842068dde) - 2026-06-08T08:14:51Z - Brad White (@bradselw) - PR: [Parameterize DartLab templates repository alias #84037](https://github.com/dotnet/roslyn/pull/84037)

- **dotnet/efcore** (`main`) - **5** commit(s)
  - [Pin GitHub workflow actions to immutable commit SHAs (#38385)](https://github.com/dotnet/efcore/commit/ab82f5e7b91906eea37296aa92c35d2897ed70b1) - 2026-06-08T20:59:27Z - Copilot (@Copilot) - PR: [Pin GitHub workflow actions to immutable commit SHAs #38385](https://github.com/dotnet/efcore/pull/38385)
  - [EF1003: Detect explicit string.Format/string.Concat in raw SQL APIs (…](https://github.com/dotnet/efcore/commit/04b37f3ed41fd685a68c9402fe19871d2e20037a) - 2026-06-08T16:58:41Z - Maxwell (@m-x-shokhzod) - PR: [EF1003: Detect explicit string.Format/string.Concat in raw SQL APIs #38208](https://github.com/dotnet/efcore/pull/38208)
  - [Translate List<T>.Exists to Queryable.Any (#38226)](https://github.com/dotnet/efcore/commit/1c5cd69729c8ac7f5a3df766286db99f49dc6031) - 2026-06-08T16:53:58Z - Maxwell (@m-x-shokhzod) - PR: [Translate List<T>.Exists to Queryable.Any #38226](https://github.com/dotnet/efcore/pull/38226)
  - [Make DateTime.Parse translation tests culture-invariant (#38384)](https://github.com/dotnet/efcore/commit/e4afabf3d42d601fe72cf307f177592adbefb998) - 2026-06-08T16:36:55Z - Arthur Vickers (@ajcvickers) - PR: [Make DateTime.Parse translation tests culture-invariant #38384](https://github.com/dotnet/efcore/pull/38384)
  - [Update dependencies from build 317483 (#38380)](https://github.com/dotnet/efcore/commit/7201613c0031141cebdc196a9d9df402667f3e9d) - 2026-06-08T15:58:29Z - dotnet-maestro[bot] (@dotnet-maestro[bot]) - PR: [[main] Source code updates from dotnet/dotnet #38380](https://github.com/dotnet/efcore/pull/38380)

- **dotnet/ef6** (`main`) - **1** commit(s)
  - [Update dependencies from https://github.com/dotnet/arcade build 20260…](https://github.com/dotnet/ef6/commit/60ef713579fe6ab7629f1e5d19791a3f2d7a9c2e) - 2026-06-08T06:55:16Z - dotnet-maestro[bot] (@dotnet-maestro[bot]) - PR: [[main] Update dependencies from dotnet/arcade #2410](https://github.com/dotnet/ef6/pull/2410)

### microsoft

- **microsoft/kiota-samples** (`main`) - **22** commit(s)
  - [Merge pull request #8714 from microsoft/dependabot/npm_and_yarn/petst…](https://github.com/microsoft/kiota-samples/commit/a69c8da95a0506b32ef3de3bfecaa467b6212cd8) - 2026-06-08T16:21:56Z - Vincent Biret (@baywet) - PR: [build(deps-dev): bump webpack from 5.107.0 to 5.107.2 in /petstore/typescript #8714](https://github.com/microsoft/kiota-samples/pull/8714)
  - [Merge pull request #8706 from microsoft/dependabot/npm_and_yarn/petst…](https://github.com/microsoft/kiota-samples/commit/46e742411789823a0af02b60a888e3866ff3603b) - 2026-06-08T16:21:10Z - Vincent Biret (@baywet) - PR: [build(deps): bump qs from 6.14.2 to 6.15.2 in /petstore/typescript #8706](https://github.com/microsoft/kiota-samples/pull/8706)
  - [build(deps): bump qs from 6.14.2 to 6.15.2 in /petstore/typescript](https://github.com/microsoft/kiota-samples/commit/e53398706591f02b7c7cd3483b853f3ee9251695) - 2026-06-08T16:20:39Z - dependabot[bot] (@dependabot[bot]) - PR: [build(deps): bump qs from 6.14.2 to 6.15.2 in /petstore/typescript #8706](https://github.com/microsoft/kiota-samples/pull/8706)
  - [Merge pull request #8716 from microsoft/dependabot/nuget/sample-api/a…](https://github.com/microsoft/kiota-samples/commit/98c91d3c59ed3f2b08abb7a64d10a845a68526b8) - 2026-06-08T16:20:18Z - Vincent Biret (@baywet) - PR: [Bump Microsoft.NET.Test.Sdk from 18.5.1 to 18.6.0 #8716](https://github.com/microsoft/kiota-samples/pull/8716)
  - [Merge pull request #8727 from microsoft/dependabot/nuget/sample-api/a…](https://github.com/microsoft/kiota-samples/commit/9962c6c31bde8792a61fa6a7557255ec014f190f) - 2026-06-08T16:19:33Z - Vincent Biret (@baywet) - PR: [Bump Swashbuckle.AspNetCore from 10.1.7 to 10.2.1 #8727](https://github.com/microsoft/kiota-samples/pull/8727)

- **microsoft/vscode-docs** (`main`) - **8** commit(s)
  - [Merge pull request #9903 from microsoft/ntrogh/fix-context-window-122](https://github.com/microsoft/vscode-docs/commit/46d3e221a6abc4c9df8662a5aa19fcd3c549430d) - 2026-06-08T20:25:42Z - Nick Trogh (@ntrogh) - PR: [Update screenshot of model picker #9903](https://github.com/microsoft/vscode-docs/pull/9903)
  - [Update screenshot of model picker](https://github.com/microsoft/vscode-docs/commit/fd7eb2980f26a90ad80710693bb402434c96213a) - 2026-06-08T19:37:13Z - Nick Trogh (@ntrogh) - PR: [Update screenshot of model picker #9903](https://github.com/microsoft/vscode-docs/pull/9903)
  - [Little grammar and clarity tweaks (#9902)](https://github.com/microsoft/vscode-docs/commit/975c3d049f44984430fed7483574bacb408e87ea) - 2026-06-08T17:30:28Z - lucd1217 (@lucd1217) - PR: [Little grammar and clarity tweaks #9902](https://github.com/microsoft/vscode-docs/pull/9902)
  - [Merge pull request #9901 from microsoft/ntrogh/getstarted-editing-tut…](https://github.com/microsoft/vscode-docs/commit/04cbc2581e238550f3f29df508bdd5ba8cff1d99) - 2026-06-08T08:58:29Z - Nick Trogh (@ntrogh) - PR: [Add cross-link to editor tutorial #9901](https://github.com/microsoft/vscode-docs/pull/9901)
  - [Add cross-link to editor tutorial](https://github.com/microsoft/vscode-docs/commit/affb3e40172cbc9d5a5043a9e0ed4fb8af96cb07) - 2026-06-08T08:44:20Z - Nick Trogh (@ntrogh) - PR: [Add cross-link to editor tutorial #9901](https://github.com/microsoft/vscode-docs/pull/9901)

- **microsoft/entrabot** (`main`) - **5** commit(s)
  - [docs: add MXC (Microsoft Execution Containers) platform-learnings res…](https://github.com/microsoft/entrabot/commit/0f8be9abdabf26388f1ee38861c10329f68bf402) - 2026-06-08T20:52:01Z - Brandon Werner (@brandwe) - PR: [docs: MXC (Microsoft Execution Containers) platform research #14](https://github.com/microsoft/entrabot/pull/14)
  - [docs: fix stale Bot Gateway code citations in CLI-port notes (#13)](https://github.com/microsoft/entrabot/commit/9e590fe203234d640ce1944ac5e8452c41da38bf) - 2026-06-08T19:36:42Z - Brandon Werner (@brandwe) - PR: [docs: fix stale Bot Gateway code citations (follow-up to #12) #13](https://github.com/microsoft/entrabot/pull/13)
  - [refactor: remove Teams Bot Gateway auth mode (ADR-006) (#12)](https://github.com/microsoft/entrabot/commit/68a1a4e4c0891eeeb738e42f0963bb8e51daddef) - 2026-06-08T17:29:32Z - Brandon Werner (@brandwe) - PR: [refactor: remove Teams Bot Gateway auth mode (ADR-006) #12](https://github.com/microsoft/entrabot/pull/12)
  - [chore(claude): allowlist persona-sati MCP tools to prevent non-intera…](https://github.com/microsoft/entrabot/commit/bdb28bcd3e2e0810a07223b52e97babb5a26b009) - 2026-06-08T00:32:27Z - Brandon Werner (@brandwe) - PR: [chore(claude): allowlist persona-sati MCP tools to prevent non-interactive REJECT #11](https://github.com/microsoft/entrabot/pull/11)
  - [Merge pull request #10 from microsoft/fix/active-sponsor-channel-binding](https://github.com/microsoft/entrabot/commit/4d5af66155b93ba8697bd250222d4e3521a20d38) - 2026-06-07T22:10:05Z - Brandon Werner (@brandwe) - PR: [fix(security): require active-sponsor-channel binding for add_teams_member and share_file #10](https://github.com/microsoft/entrabot/pull/10)

- **microsoft/kiota** (`main`) - **3** commit(s)
  - [Bump Spectre.Console from 0.55.2 to 0.56.0 (#7770)](https://github.com/microsoft/kiota/commit/369efd30c140db0e91d372d7504182c7d778c85e) - 2026-06-08T11:19:19Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump Spectre.Console from 0.55.2 to 0.56.0 #7770](https://github.com/microsoft/kiota/pull/7770)
  - [build(deps-dev): bump @types/node in /it/typescript (#7768)](https://github.com/microsoft/kiota/commit/f873a879e79802b58ed1fb93f3f332c3b7cac357) - 2026-06-08T10:23:42Z - dependabot[bot] (@dependabot[bot]) - PR: [build(deps-dev): bump @types/node from 25.9.1 to 25.9.2 in /it/typescript #7768](https://github.com/microsoft/kiota/pull/7768)
  - [build(deps-dev): bump @types/node from 25.9.1 to 25.9.2 in /vscode (#…](https://github.com/microsoft/kiota/commit/39939a54052a8d555607609dd98b11e8e4c7d8de) - 2026-06-08T10:00:49Z - dependabot[bot] (@dependabot[bot]) - PR: [build(deps-dev): bump @types/node from 25.9.1 to 25.9.2 in /vscode #7769](https://github.com/microsoft/kiota/pull/7769)

- **microsoft/WindowsAppSDK** (`main`) - **2** commit(s)
  - [Add Templates PR validation pipeline (dotnet new + VSIX) (#6526)](https://github.com/microsoft/WindowsAppSDK/commit/9bde0388e0177b11a012d8b475eb1edeabebcccc) - 2026-06-08T18:43:48Z - Dinah Xiaoman G (@DinahK-2SO) - PR: [Add Templates PR validation pipeline (dotnet new + VSIX) #6526](https://github.com/microsoft/WindowsAppSDK/pull/6526)
  - [[WinAppSDK CMake Support]: Implicitly choose deployment mode based on…](https://github.com/microsoft/WindowsAppSDK/commit/f9f3c39dfd64d66dc1fbe2baecda5288c705e2b4) - 2026-06-08T16:38:03Z - ssparach (@ssparach) - PR: [[WinAppSDK CMake Support]: Implicitly choose deployment mode based on WindowsAppSDKSelfContained #6490](https://github.com/microsoft/WindowsAppSDK/pull/6490)

- **microsoft/kiota-typescript** (`main`) - **2** commit(s)
  - [Merge pull request #2040 from microsoft/dependabot/npm_and_yarn/micro…](https://github.com/microsoft/kiota-typescript/commit/cf22bfccfd28f500e777ffb762be5554a04ce1ac) - 2026-06-08T15:07:55Z - github-actions[bot] (@github-actions[bot]) - PR: [auto dependabot: bump @microsoft/sp-http from 1.23.0 to 1.23.1 #2040](https://github.com/microsoft/kiota-typescript/pull/2040)
  - [auto dependabot: bump @microsoft/sp-http from 1.23.0 to 1.23.1](https://github.com/microsoft/kiota-typescript/commit/e0c8a76a4a00b1d2869316500d5773d656d63c6a) - 2026-06-08T14:33:34Z - dependabot[bot] (@dependabot[bot]) - PR: [auto dependabot: bump @microsoft/sp-http from 1.23.0 to 1.23.1 #2040](https://github.com/microsoft/kiota-typescript/pull/2040)

- **microsoft/kiota-java** (`main`) - **2** commit(s)
  - [Merge pull request #2122 from microsoft/dependabot/gradle/open-teleme…](https://github.com/microsoft/kiota-java/commit/4ae738c1b853e1bc0ea51befbacd7e0aee17f355) - 2026-06-08T10:00:25Z - github-actions[bot] (@github-actions[bot]) - PR: [chore(deps): bump the open-telemetry group across 1 directory with 2 updates #2122](https://github.com/microsoft/kiota-java/pull/2122)
  - [chore(deps): bump the open-telemetry group across 1 directory with 2 …](https://github.com/microsoft/kiota-java/commit/bcdd25c6177b24d0d2b38bd52831a48801bb0024) - 2026-06-08T09:05:13Z - dependabot[bot] (@dependabot[bot]) - PR: [chore(deps): bump the open-telemetry group across 1 directory with 2 updates #2122](https://github.com/microsoft/kiota-java/pull/2122)

- **microsoft/playwright.dev** (`main`) - **1** commit(s)
  - [chore(deps): bump glob from 11.0.0 to 11.1.0 (#2050)](https://github.com/microsoft/playwright.dev/commit/d6064db2f5bf6b5f4d2b7dd0ab9bf81b0a46e20b) - 2026-06-08T19:58:35Z - dependabot[bot] (@dependabot[bot]) - PR: [chore(deps): bump glob from 11.0.0 to 11.1.0 #2050](https://github.com/microsoft/playwright.dev/pull/2050)

- **microsoft/aspire-samples** (`main`) - **1** commit(s)
  - [UX revamp: bespoke, accessible identities for every Aspire sample (#1…](https://github.com/microsoft/aspire-samples/commit/563499a1ce016d0c352bf20f9491ab3af05c9e6f) - 2026-06-08T17:56:14Z - David Pine (@IEvangelist) - PR: [UX revamp: bespoke, accessible identities for every Aspire sample #1682](https://github.com/microsoft/aspire-samples/pull/1682)

- **microsoft/azure-quantum-python** (`main`) - **1** commit(s)
  - [Update deprecated requirement in /azure-quantum (#746)](https://github.com/microsoft/azure-quantum-python/commit/5766d606913fec8cf7075ffab444fd8004559f94) - 2026-06-08T17:08:18Z - dependabot[bot] (@dependabot[bot]) - PR: [Update deprecated requirement from <2.0,>=1.2.12 to >=1.3.1,<2.0 in /azure-quantum #746](https://github.com/microsoft/azure-quantum-python/pull/746)

- **microsoft/kiota-python** (`main`) - **1** commit(s)
  - [Merge pull request #566 from microsoft/release-please--branches--main](https://github.com/microsoft/kiota-python/commit/f04d4f8773e3ccade924fec2da05ef0756ba082c) - 2026-06-08T16:35:22Z - Vincent Biret (@baywet) - PR: [chore: release main #566](https://github.com/microsoft/kiota-python/pull/566)

- **microsoft/amplifier-docs** (`main`) - **1** commit(s)
  - [docs: Regenerate redaction.md with evidence validation](https://github.com/microsoft/amplifier-docs/commit/00a617ade362fee179de0525c1daea82b5667598) - 2026-06-08T08:11:50Z - github-actions[bot] (@github-actions[bot])

### microsoftgraph

- **microsoftgraph/msgraph-snippets-go** (`main`) - **2** commit(s)
  - [Merge pull request #214 from microsoftgraph/dependabot/go_modules/src…](https://github.com/microsoftgraph/msgraph-snippets-go/commit/56fbf186b82e698360a52e80a0d2ed43d6c80244) - 2026-06-08T16:26:10Z - github-actions[bot] (@github-actions[bot]) - PR: [Bump github.com/Azure/azure-sdk-for-go/sdk/azcore from 1.21.1 to 1.22.0 in /src #214](https://github.com/microsoftgraph/msgraph-snippets-go/pull/214)
  - [Bump github.com/Azure/azure-sdk-for-go/sdk/azcore in /src](https://github.com/microsoftgraph/msgraph-snippets-go/commit/d5dad26e59d368e9415084676daccb045b6389ea) - 2026-06-08T16:23:05Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump github.com/Azure/azure-sdk-for-go/sdk/azcore from 1.21.1 to 1.22.0 in /src #214](https://github.com/microsoftgraph/msgraph-snippets-go/pull/214)

## samples

### Azure

- **Azure/agentops** (`main`) - **27** commit(s)
  - [Merge pull request #265 from Azure/release/v0.3.11](https://github.com/Azure/agentops/commit/2b46ef990dacfbf685de40136deb9cae6ae76fe2) - 2026-06-08T20:12:12Z - Paulo Lacerda (@placerda) - PR: [Release v0.3.11 #265](https://github.com/Azure/agentops/pull/265)
  - [fix: support reasoning grader eval runs](https://github.com/Azure/agentops/commit/dc910e428f799e77c4fe414b6dbf6828e3c6510d) - 2026-06-08T19:39:48Z - Paulo Lacerda (@placerda) - PR: [Release v0.3.11 #265](https://github.com/Azure/agentops/pull/265)
  - [fix: show heartbeat during azd eval runs](https://github.com/Azure/agentops/commit/f8c0dd6ab4296183d044485855143abcc9200030) - 2026-06-08T18:00:57Z - Paulo Lacerda (@placerda) - PR: [fix: show heartbeat during azd eval runs #264](https://github.com/Azure/agentops/pull/264)
  - [fix: recommend eval init before azd eval run](https://github.com/Azure/agentops/commit/9381b5362aa9e7a7a7551503af2bbb6fafbe5284) - 2026-06-08T17:49:38Z - Paulo Lacerda (@placerda) - PR: [fix: recommend eval init before azd eval run #263](https://github.com/Azure/agentops/pull/263)
  - [fix: bootstrap azd eval init context](https://github.com/Azure/agentops/commit/9180b7921fff2c0da7fc77ef10eb569c74a2f627) - 2026-06-08T17:40:13Z - Paulo Lacerda (@placerda) - PR: [fix: bootstrap azd eval init context #262](https://github.com/Azure/agentops/pull/262)

- **Azure/aks-rdma-infiniband** (`main`) - **2** commit(s)
  - [chore(deps): update vllm/vllm-openai docker tag to v0.22.1](https://github.com/Azure/aks-rdma-infiniband/commit/832342cc64f01d3563972acfbd25c18a7753dfe8) - 2026-06-08T16:30:57Z - renovate[bot] (@renovate[bot]) - PR: [chore(deps): update Docker base images (nvidia/cuda, vllm-openai) #160](https://github.com/Azure/aks-rdma-infiniband/pull/160)
  - [chore(deps): update nvidia/cuda docker tag to v13.3.0](https://github.com/Azure/aks-rdma-infiniband/commit/950ac7e0d5ba76cf0e7c42d9c9239084ad686897) - 2026-06-08T16:30:57Z - renovate[bot] (@renovate[bot]) - PR: [chore(deps): update Docker base images (nvidia/cuda, vllm-openai) #160](https://github.com/Azure/aks-rdma-infiniband/pull/160)

### Azure-Samples

- **Azure-Samples/AIEngineerPersona** (`main`) - **1** commit(s)
  - [Surprise Me UX: visible spinner, no-shift status slot, a11y](https://github.com/Azure-Samples/AIEngineerPersona/commit/44d5235de5562c5f6bf21ab2940bbb75a3b224a7) - 2026-06-08T19:01:36Z - asofio (@asofio)

- **Azure-Samples/postgres-sa-byoac** (`main`) - **1** commit(s)
  - [fixed 12 vulnerabilities across aml model, backend, and frontend](https://github.com/Azure-Samples/postgres-sa-byoac/commit/7f2635de3a9f0b36fca2116cc46177096336a0fe) - 2026-06-08T17:09:28Z - jjfrost (@jjfrost)

- **Azure-Samples/aks-store-demo** (`main`) - **1** commit(s)
  - [chore: update actions/checkout and Trivy action versions in audit-ter…](https://github.com/Azure-Samples/aks-store-demo/commit/43f7206d7299945575920bc91159e2f37242f0d0) - 2026-06-08T01:22:50Z - Paul Yu (@pauldotyu)

- **Azure-Samples/azure-sdk-for-js-docs** (`main`) - **1** commit(s)
  - [Bump @azure/storage-blob (#89)](https://github.com/Azure-Samples/azure-sdk-for-js-docs/commit/8e30023bdef6ac9732660943ecc8aff1dee08d2f) - 2026-06-08T00:24:43Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump @azure/storage-blob from 12.31.0 to 12.32.0 in /samples/identity/credentials in the azure-sdk group #89](https://github.com/Azure-Samples/azure-sdk-for-js-docs/pull/89)

### OfficeDev

- **OfficeDev/Microsoft-Teams-Samples** (`main`) - **2** commit(s)
  - [Merge pull request #2364 from OfficeDev/v-ajayjadhav/readmerename](https://github.com/OfficeDev/Microsoft-Teams-Samples/commit/987bab8e71aa790a59dfb98666bffd2ce83b8a76) - 2026-06-08T08:54:01Z - Harikrishnan-MSFT (@Harikrishnan-MSFT) - PR: [Rename Readme.md to README.md for graph-rsc-helper sample #2364](https://github.com/OfficeDev/Microsoft-Teams-Samples/pull/2364)
  - [Rename Readme.md to README.md](https://github.com/OfficeDev/Microsoft-Teams-Samples/commit/35c999afd06949e515a3d835f896ab709ee0f370) - 2026-06-08T08:48:31Z - AjayJ12-MSFT (@AjayJ12-MSFT) - PR: [Rename Readme.md to README.md for graph-rsc-helper sample #2364](https://github.com/OfficeDev/Microsoft-Teams-Samples/pull/2364)

### microsoft

- **microsoft/teams-agent-accelerator-templates** (`main`) - **9** commit(s)
  - [Bump orjson from 3.10.15 to 3.11.6 in /python/web-browsing-agent (#99)](https://github.com/microsoft/teams-agent-accelerator-templates/commit/b8e88a684faa46e20e56ea82fcab7f85df0143cb) - 2026-06-08T19:11:13Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump orjson from 3.10.15 to 3.11.6 in /python/web-browsing-agent #99](https://github.com/microsoft/teams-agent-accelerator-templates/pull/99)
  - [Bump pyasn1 from 0.6.1 to 0.6.3 in /python/web-browsing-agent (#98)](https://github.com/microsoft/teams-agent-accelerator-templates/commit/b3b9e53222f330a10c9de394508ae71a731605fd) - 2026-06-08T18:56:07Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump pyasn1 from 0.6.1 to 0.6.3 in /python/web-browsing-agent #98](https://github.com/microsoft/teams-agent-accelerator-templates/pull/98)
  - [Bump requests from 2.32.3 to 2.33.0 in /python/web-browsing-agent (#97)](https://github.com/microsoft/teams-agent-accelerator-templates/commit/fe5e8a31025d12691f78870f1f8e08c934d4753f) - 2026-06-08T17:26:19Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump requests from 2.32.3 to 2.33.0 in /python/web-browsing-agent #97](https://github.com/microsoft/teams-agent-accelerator-templates/pull/97)
  - [Bump cryptography from 44.0.0 to 46.0.7 in /python/web-browsing-agent…](https://github.com/microsoft/teams-agent-accelerator-templates/commit/7cfd3b8a1ca683ea390afcad5bf1e25e419b2379) - 2026-06-08T17:21:01Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump cryptography from 44.0.0 to 46.0.7 in /python/web-browsing-agent #96](https://github.com/microsoft/teams-agent-accelerator-templates/pull/96)
  - [Bump next from 15.5.7 to 15.5.18 in /docs (#83)](https://github.com/microsoft/teams-agent-accelerator-templates/commit/5075185f975100e0410b2ceb6b0d5ba9cb78abeb) - 2026-06-08T17:18:21Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump next from 15.5.7 to 15.5.18 in /docs #83](https://github.com/microsoft/teams-agent-accelerator-templates/pull/83)

- **microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools** (`main`) - **7** commit(s)
  - [Merge pull request #16 from microsoft/santiagxf/refactor](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/commit/81c2ccd41c689bf98cc1db317be03d55e01a803d) - 2026-06-08T00:35:21Z - Facundo Santiago (@santiagxf) - PR: [fix: server #16](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/pull/16)
  - [fix: server](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/commit/d915b924123c7026646d15b0d0dbf30ad4c07aa5) - 2026-06-08T00:34:50Z - santiagxf (@santiagxf) - PR: [fix: server #16](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/pull/16)
  - [Merge pull request #15 from microsoft/santiagxf/refactor](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/commit/352afc259f43eeb54d0f095559a294068e87938d) - 2026-06-08T00:33:15Z - Facundo Santiago (@santiagxf) - PR: [fix: refactor MCP and auth #15](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/pull/15)
  - [fix: refactor MCP and auth](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/commit/0c276a29a0aaf9191ed6ed7883cb046f00a7f578) - 2026-06-08T00:29:20Z - santiagxf (@santiagxf) - PR: [fix: refactor MCP and auth #15](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/pull/15)
  - [Merge pull request #13 from microsoft/santiagxf/refactor](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/commit/e532fac56baff32098d978de85a31d73e1681803) - 2026-06-07T21:54:42Z - Facundo Santiago (@santiagxf) - PR: [Refactor agent and MCP server integration #13](https://github.com/microsoft/Build26-DEM333-how-foundry-integrates-with-open-source-frameworks-and-tools/pull/13)

- **microsoft/onnxruntime** (`main`) - **4** commit(s)
  - [Clarify and improve ownership semantics Model Editor C/C++ API (#28800)](https://github.com/microsoft/onnxruntime/commit/dadd2d031b103c6d51831509ab7c228c543a5525) - 2026-06-08T20:29:36Z - Dmitri Smirnov (@yuslepukhin) - PR: [Clarify and improve ownership semantics Model Editor C/C++ API #28800](https://github.com/microsoft/onnxruntime/pull/28800)
  - [Fix Type-confusion: raw-pointer bind_input causing OOB write (#28839)](https://github.com/microsoft/onnxruntime/commit/7bf3032bc0f5c79680e9f5a1fa6c9bca590a9036) - 2026-06-08T19:23:53Z - Selena Yang (@selenayang888) - PR: [Fix Type-confusion: raw-pointer bind_input causing OOB write #28839](https://github.com/microsoft/onnxruntime/pull/28839)
  - [[CoreML EP] Add Where and And builders (#28597)](https://github.com/microsoft/onnxruntime/commit/073e49ffe1b70e61ea5e5cf4dec17ea9879914f8) - 2026-06-08T17:08:22Z - Max Buckley (@maxwbuckley) - PR: [[CoreML EP] Add Where and And builders #28597](https://github.com/microsoft/onnxruntime/pull/28597)
  - [fix(ci): nodejs on nuget cuda 13 hangs (#28816)](https://github.com/microsoft/onnxruntime/commit/8f6e944b8658ef146593e7852d984536dee37d3e) - 2026-06-08T14:29:05Z - Sanaa Hamel (@sanaa-hamel-microsoft) - PR: [fix(ci): nodejs on nuget cuda 13 hangs #28816](https://github.com/microsoft/onnxruntime/pull/28816)

- **microsoft/dragon-copilot-extension-samples** (`main`) - **2** commit(s)
  - [deps(deps-dev): bump @types/node in /tools/dragon-copilot-cli (#244)](https://github.com/microsoft/dragon-copilot-extension-samples/commit/c72bd288b8facac7c67ed153c9c0a01c1115980e) - 2026-06-08T14:38:14Z - dependabot[bot] (@dependabot[bot]) - PR: [deps(deps-dev): bump @types/node from 25.9.1 to 25.9.2 in /tools/dragon-copilot-cli #244](https://github.com/microsoft/dragon-copilot-extension-samples/pull/244)
  - [deps: Bump Swashbuckle.AspNetCore from 10.2.0 to 10.2.1 (#245)](https://github.com/microsoft/dragon-copilot-extension-samples/commit/3e97c65609698f6ed901c85da8fd5427991d6f06) - 2026-06-08T14:37:15Z - dependabot[bot] (@dependabot[bot]) - PR: [deps: Bump Swashbuckle.AspNetCore from 10.2.0 to 10.2.1 #245](https://github.com/microsoft/dragon-copilot-extension-samples/pull/245)

- **microsoft/DirectX-SDK-Samples** (`main`) - **1** commit(s)
  - [Bump actions/checkout from 6.0.2 to 6.0.3 (#14)](https://github.com/microsoft/DirectX-SDK-Samples/commit/b9656e7e13efc531bfb6dacb18f4032f4c125bda) - 2026-06-08T18:35:15Z - dependabot[bot] (@dependabot[bot]) - PR: [Bump actions/checkout from 6.0.2 to 6.0.3 #14](https://github.com/microsoft/DirectX-SDK-Samples/pull/14)

- **microsoft/AZD-for-beginners** (`main`) - **1** commit(s)
  - [Merge pull request #98 from microsoft/copilot/fix-japanese-translation](https://github.com/microsoft/AZD-for-beginners/commit/10a2706ae895399ce58be8e972d23caa0943d3d2) - 2026-06-08T11:58:17Z - Lee Stott (@leestott) - PR: [Translate untranslated English headings in Japanese changelog #98](https://github.com/microsoft/AZD-for-beginners/pull/98)

- **microsoft/shell-intune-samples** (`master`) - **1** commit(s)
  - [Merge pull request #279 from microsoft/neils-updates](https://github.com/microsoft/shell-intune-samples/commit/d8be42cd646f2c61cfab58ad35ce980faba11d68) - 2026-06-08T10:51:33Z - Chris Kunze (MSFT) (@CKunze-MSFT) - PR: [Pin Microsoft Edge repo to legacy microsoft.gpg key (fix 26.04 signature failure) #279](https://github.com/microsoft/shell-intune-samples/pull/279)

- **microsoft/Document-Knowledge-Mining-Solution-Accelerator** (`main`) - **1** commit(s)
  - [Merge pull request #656 from microsoft/dev](https://github.com/microsoft/Document-Knowledge-Mining-Solution-Accelerator/commit/a87c02473d01cc9809d8bc9898c09c2faa858890) - 2026-06-08T05:47:24Z - Roopan-Microsoft (@Roopan-Microsoft) - PR: [feat: merging dev to main #656](https://github.com/microsoft/Document-Knowledge-Mining-Solution-Accelerator/pull/656)

- **microsoft/agentic-applications-for-unified-data-foundation-solution-accelerator** (`main`) - **1** commit(s)
  - [Merge pull request #386 from microsoft/dev](https://github.com/microsoft/agentic-applications-for-unified-data-foundation-solution-accelerator/commit/a6f6d1ccccead20c4a4a415ede13b5a3f1398b2a) - 2026-06-08T05:47:17Z - Roopan-Microsoft (@Roopan-Microsoft) - PR: [chore: dev to main merge #386](https://github.com/microsoft/agentic-applications-for-unified-data-foundation-solution-accelerator/pull/386)

- **microsoft/Conversation-Knowledge-Mining-Solution-Accelerator** (`main`) - **1** commit(s)
  - [Merge pull request #953 from microsoft/dev](https://github.com/microsoft/Conversation-Knowledge-Mining-Solution-Accelerator/commit/f4cb41c836c243c1ede9505fcc43599737f7335a) - 2026-06-08T05:46:58Z - Roopan-Microsoft (@Roopan-Microsoft) - PR: [chore: dev to main merge #953](https://github.com/microsoft/Conversation-Knowledge-Mining-Solution-Accelerator/pull/953)

- **microsoft/content-generation-solution-accelerator** (`main`) - **1** commit(s)
  - [Merge pull request #866 from microsoft/dev](https://github.com/microsoft/content-generation-solution-accelerator/commit/bcdb2807f2c5cb9b028cbd99f97cfba53994e2ac) - 2026-06-08T05:44:37Z - Roopan-Microsoft (@Roopan-Microsoft) - PR: [chore: dev to main merge #866](https://github.com/microsoft/content-generation-solution-accelerator/pull/866)

- **microsoft/BCTech** (`master`) - **1** commit(s)
  - [Merge pull request #355 from microsoft/workshops/agent-building](https://github.com/microsoft/BCTech/commit/448d5e660d9559b3df40fc075253536aa751b9f4) - 2026-06-08T01:14:13Z - Thaddeus Loke (@thloke) - PR: [Add 'Building Agents in Business Central' workshop #355](https://github.com/microsoft/BCTech/pull/355)

## training

### MicrosoftLearning

- **MicrosoftLearning/ai-apps** (`main`) - **6** commit(s)
  - [Force wllama to CPU](https://github.com/MicrosoftLearning/ai-apps/commit/ee318cfe6c687ee8ef1a39ecc75c91a67f33f12b) - 2026-06-08T17:11:43Z - Graeme Malcolm (@GraemeMalcolm)
  - [Reduce history](https://github.com/MicrosoftLearning/ai-apps/commit/11ae239b8caab1b0556f4486da025f10909b7b97) - 2026-06-08T16:46:55Z - Graeme Malcolm (@GraemeMalcolm)
  - [Harden system prompt](https://github.com/MicrosoftLearning/ai-apps/commit/3357909bea3b8a6d0b49cb7e654edf3640168354) - 2026-06-08T16:44:13Z - Graeme Malcolm (@GraemeMalcolm)
  - [Reduced WebLLM context window](https://github.com/MicrosoftLearning/ai-apps/commit/5a440bd9dc931145fd6cefe99cc6e311ea1f485d) - 2026-06-08T16:31:25Z - Graeme Malcolm (@GraemeMalcolm)
  - [Optimization](https://github.com/MicrosoftLearning/ai-apps/commit/54dfb8427601626ed5bc59cd330c46641352980a) - 2026-06-08T00:24:52Z - Graeme Malcolm (@GraemeMalcolm)

- **MicrosoftLearning/mslearn-azure-ai.ja-jp** (`main`) - **2** commit(s)
  - [Auto-generated zip files [skip ci]](https://github.com/MicrosoftLearning/mslearn-azure-ai.ja-jp/commit/77e78b6612efa171ac6d23f515d47a3493a3f585) - 2026-06-08T07:46:48Z - GitHub Action (@actions-user)
  - [Copy Files From Source Repo (2026-06-08 00:46)](https://github.com/MicrosoftLearning/mslearn-azure-ai.ja-jp/commit/3c587da3a6310015ccfb321cbdf30ae896747060) - 2026-06-08T07:46:32Z - olprod (@olprod)

- **MicrosoftLearning/mslearn-sql-developer.ja-jp** (`main`) - **2** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:46)](https://github.com/MicrosoftLearning/mslearn-sql-developer.ja-jp/commit/fefbe1243e8e1ef43e045d5231cb5060e113c2a0) - 2026-06-08T07:46:40Z - olprod (@olprod)
  - [Initial commit](https://github.com/MicrosoftLearning/mslearn-sql-developer.ja-jp/commit/4900f8bdf8f03fa260499d48468ef467c04b6e6e) - 2026-06-08T02:51:46Z - stonezy123 (@stonezy123)

- **MicrosoftLearning/scenario-sims** (`master`) - **1** commit(s)
  - [add model select sim](https://github.com/MicrosoftLearning/scenario-sims/commit/e3a80414cfb421313ccf5da02dd3142c1a95dc66) - 2026-06-08T17:27:46Z - madiepev (@madiepev)

- **MicrosoftLearning/mslearn-ai-agents** (`main`) - **1** commit(s)
  - [Revise integration steps for Foundry IQ](https://github.com/MicrosoftLearning/mslearn-ai-agents/commit/dadd2a794f9284f6fbfbc93be02a46f9a1779aa1) - 2026-06-08T17:17:27Z - Ivor Berry (@ivorb)

- **MicrosoftLearning/SC-200T00A-Microsoft-Security-Operations-Analyst.ja-jp** (`main`) - **1** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:43)](https://github.com/MicrosoftLearning/SC-200T00A-Microsoft-Security-Operations-Analyst.ja-jp/commit/92286e96b9913afb992bc2af5fa3ff043ba97d72) - 2026-06-08T07:43:52Z - olprod (@olprod)

- **MicrosoftLearning/mslearn-ai-fundamentals.ja-jp** (`main`) - **1** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:39)](https://github.com/MicrosoftLearning/mslearn-ai-fundamentals.ja-jp/commit/a6c404c56ab81e668c83ca28d70f10e1081c6deb) - 2026-06-08T07:39:29Z - olprod (@olprod)

- **MicrosoftLearning/mslearn-ai-agents.ja-jp** (`main`) - **1** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:39)](https://github.com/MicrosoftLearning/mslearn-ai-agents.ja-jp/commit/fb0d96793e08e5ec165a7d09344370f0e3d4c7b8) - 2026-06-08T07:39:04Z - olprod (@olprod)

- **MicrosoftLearning/MS-4022-Extend-Microsoft-365-Copilot-in-Copilot-Studio.ja-jp** (`main`) - **1** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:38)](https://github.com/MicrosoftLearning/MS-4022-Extend-Microsoft-365-Copilot-in-Copilot-Studio.ja-jp/commit/3fa79d63cd79bfadb963169aa2a1459a9a5adcf3) - 2026-06-08T07:38:50Z - olprod (@olprod)

- **MicrosoftLearning/MS-4021-Copilot-Immersion-Experience.ja-jp** (`main`) - **1** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:38)](https://github.com/MicrosoftLearning/MS-4021-Copilot-Immersion-Experience.ja-jp/commit/2a145b1ed1699db8f71a10b1b2acca9b1c377191) - 2026-06-08T07:38:41Z - olprod (@olprod)

- **MicrosoftLearning/MS-4010-Build-Plugins-Connectors-Microsoft-Copilot-M365.ja-jp** (`main`) - **1** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:37)](https://github.com/MicrosoftLearning/MS-4010-Build-Plugins-Connectors-Microsoft-Copilot-M365.ja-jp/commit/ab433959df05bd9ddd5cc00e0a7c8da5dedd0db5) - 2026-06-08T07:37:42Z - olprod (@olprod)

- **MicrosoftLearning/AZ-700-Designing-and-Implementing-Microsoft-Azure-Networking-Solutions.ja-jp** (`main`) - **1** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:34)](https://github.com/MicrosoftLearning/AZ-700-Designing-and-Implementing-Microsoft-Azure-Networking-Solutions.ja-jp/commit/3f6b020bf10efa93fd5254e005823ee09bdd60a6) - 2026-06-08T07:34:30Z - olprod (@olprod)

- **MicrosoftLearning/AZ-104-MicrosoftAzureAdministrator.ja-jp** (`main`) - **1** commit(s)
  - [Copy Files From Source Repo (2026-06-08 00:29)](https://github.com/MicrosoftLearning/AZ-104-MicrosoftAzureAdministrator.ja-jp/commit/ec431a1621e5005536ea2dbf07226ef67cdef708) - 2026-06-08T07:29:30Z - olprod (@olprod)

- **MicrosoftLearning/mslearn-ai-concepts** (`main`) - **1** commit(s)
  - [Updated model versions](https://github.com/MicrosoftLearning/mslearn-ai-concepts/commit/0ca891eb637e3f5e782fce09c5a67c1eed9a7b3f) - 2026-06-08T00:27:10Z - Graeme Malcolm (@GraemeMalcolm)

### OfficeDev

- **OfficeDev/office-js-snippets** (`main`) - **1** commit(s)
  - [[Word] (Content Control) Add a sample for the date picker content con…](https://github.com/OfficeDev/office-js-snippets/commit/94ef35521aa46e88412148c66ae376112b930b33) - 2026-06-08T16:40:24Z - Alex Jerabek (@AlexJerabek) - PR: [[Word] (Content Control) Add a sample for the date picker content control #1095](https://github.com/OfficeDev/office-js-snippets/pull/1095)

